# Note: This file is prefixed with a number to force compile order. RubyMotion 1.32 appears to have a bug
# whereby the dependency is not properly identified.
...
module MotionModel
  class SQLContext
    ...
    def log(msg)
      @options[:logger].send(msg) if @options[:logger]
      NSLog(msg) unless @options[:ns_log] == false
    end

    def execute
      log("ExecSQL: #{@sql}") # TODO debug only
      result = @db_adapter.execute_sql(self)
      log("ExecSQLResult: #{result}") # TODO debug only
      result
    end
  end
